...
const unsigned char ASM_SIZE = 3;
// Current ASM byte marker position
unsigned int asmByteMarker = 0;
...
void loop() {
  ...
  if(newDataRead == READ_BUFFER) {
    ...
    for(unsigned int i = 0; i < newDataRead; i++) {
      blinkLed(2);
...
void sendFrameBuffer(unsigned int numBytes) {
...
void clearByteArray(byte array[], unsigned int arraySize) {
